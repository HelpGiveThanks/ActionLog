...
If [ $found = 0 ]
Show Custom Dialog [ Message: "There is no library ﬁle in memory. Open one up manually, and it will be added to memory and
opened when you next click the library button."; Buttons: “OK” ]
Else
Open URL [ Case ( Get ( SystemPlatform ) = - 2 ;
Substitute (
...
& "library/library.USR"
 ; " " ; "%20" ) ) ]
[ No dialog ]
February 2, 平成26 13:22:16 ActionLog[Conﬂict].fp7 - gotoLibraryDatabase -1-navigation: gotoLibraryDatabase
Open URL [ Case ( Get ( SystemPlatform ) = - 2 ;
Substitute (
Substitute (Left (Get (FilePath) ; Length ( Get (FilePath) ) - ( Length ( Get (FileName) ) + 4 ) ) ; "ﬁle:/" ; "ﬁle:/" )
& "library/library.USR"
...
End If
If [ Get (LastError) = 5 ]
Show Custom Dialog [ Message: "The library you last looked at cannot be opened because its name was changed, the ﬁle
was moved, or deleted. Open a library ﬁle up manually, and it will be added to memory and opened when you next
click the library button."; Buttons: “OK” ]
End If
End If
End If
Else If [ Get (LastError) = 112 and MemorySwitch::currentLibraryPath = "" ]
Show Custom Dialog [ Message: "There is no library ﬁle in memory. Open one up, and it will be added to memory to be opened in when
you next click the library button."; Buttons: “OK” ]
End If
...
Select Window [ Name: "Report" ]
Select Window [ Name: "References" ]
February 2, 平成26 13:22:16 ActionLog[Conﬂict].fp7 - gotoLibraryDatabase -2-