...
If [ Get (LastError) = 112 and MemorySwitch::currentLibraryPath ≠ "" ]
Open URL [ Case ( Get ( SystemPlatform ) = - 2 ;
Substitute (MemorySwitch::currentLibraryPath ; " " ; "%20" ) ;
Substitute ( Substitute ( MemorySwitch::currentLibraryPath ; "file:/" ; "file://" ) ; " " ; "%20" ) ) ] [ No dialog ]
#
...
#options.
If [ Get (LastError) = 5 ]
Show Custom Dialog [ Message: "Last library used cannot be found as it must have been renamed, moved, or deleted. Find and re- open it manually or click the allsolutions button to select another library."; Buttons: “OK” ]
End If
#
...
February 1, 平成26 18:15:55
ActionLog.fp7 - gotoLibraryDatabase -1-
